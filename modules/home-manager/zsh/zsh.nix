  ...
  config = lib.mkIf config.shells.zsh.enable {
    programs.zsh = {
      ...
      initContent = ''
        ...
        git_branch() {
          command git rev-parse --abbrev-ref HEAD 2>/dev/null | sed 's/.*/(&)/'
        }

        nix_indicator() {
          [[ -n $IN_NIX_SHELL ]] && echo "%F{white}❄%F{cyan} "
        }

        PS1='%F{cyan}[$(nix_indicator)%F{yellow}%n%F{green}@%F{blue}%m %F{magenta}%~%F{cyan}]%F{red}$(git_branch)%F{cyan}%(1j.(%j).)%f$ '
